...
*   **平台支持**：知乎、Bilibili、Reddit、Twitter、小红书 + 通用网页
    > **✨ 关于小红书支持的特别鸣谢**：为保障下载稳定性，告别上游 Git 历史变动导致的失效问题，本项目深度剥离并内置了特定的可用版本。该部分核心功能由开源项目 [XHS-Downloader](https://github.com/JoeanAmier/XHS-Downloader)（作者：JoeanAmier）驱动，在此向原作者致以诚挚的感谢。该组件源码遵循 **GPL-3.0 协议**，本仓库以附带源码形式完全遵守并继承其开源精神发布。
...
*   **快速导入**：首页表单支持粘贴 URL 或分享文本，自动提取链接并检测视频/网页类型【新】
*   **存档浏览**：/archives 页面统一管理视频和网页，支持排序和筛选【新】
...
- **摘要展示**：列表和预览卡片显示完整摘要文本，不再自动追加省略号
...
MemoryIndex 核心代码及架构采用 [GNU 通用公共许可证 v3.0](LICENSE)。

特别鸣谢开源社区的力量，本项目中涉及的优质模块包含：
*   **小红书下载与解析**：内置集成并修改自开源项目 [XHS-Downloader](https://github.com/JoeanAmier/XHS-Downloader) (原作者 **JoeanAmier**)。该组件代码同样被严格限制并完全遵循相应的 GPL-3.0 协议进行附带开源传播。
