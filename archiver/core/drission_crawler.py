...
文件夹名称："""
    ...
    def _generate_report_with_llm(
        ...
        # 1. 估算 Token 数量
        estimated_tokens = self._estimate_token_count(archive_content)
        total_chars = len(archive_content)
        ...
        if use_gemini:
             ...
             gemini_prompt = f"""请基于以下网页归档文本，生成一份**极致详细、内容全面**的深度内容概括。
...
"""
            ...
            try:
                ...
                content_limit = 35000
                final_archive_content = archive_content
                
                if len(archive_content) > content_limit:
                    logger.warning(f"⚠️  文本过长 ({len(archive_content)} 字符)，Groq 模式下截断至 {content_limit} 字符")
                    final_archive_content = archive_content[:content_limit] + "\n\n...(内容已截断，建议配置 Gemini 处理长文本)"
